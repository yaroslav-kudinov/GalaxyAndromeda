...
import { MAX_FLEET_SIZE_PER_PLAYER, PHASE_LABELS, SHIP_LABELS } from '@galaxy/rules'
...
// Раньше клиент держал свой список названий фаз, и он разошёлся с правилами:
// для снятой фазы производства здесь было «Действия», а в правилах — «Производство».
export { PHASE_LABELS }
...
export function gameHelpForPhase(
  ...
  if (normalized === 'planning') {
    return {
      ...
      steps: [
        {
          icon: 'queue',
          label: 'Очередь на ход — случайная',
          detail: 'Порядок игроков разыгрывается в начале хода и один на планирование и действия.',
        },
        {
          icon: 'marker-action',
          label: 'Щелчок по флоту ставит маркер',
          detail: 'Щёлкните клетку со своим кораблём, чтобы поставить или снять маркер действия — он обведён жёлтым.',
        },
        {
          icon: 'limit',
        ...
        {
          icon: 'build',
          label: 'Строить можно в фазе «Действия»',
          detail:
            'Щелчок по маркеру действия открывает окно: можно переместить корабли или построить новые на клетке маркера, оплатив фишками региона.',
        },
      ],
    }
  }
  ...
  return {
    ...
    steps: [
      {
        icon: 'queue',
        label: 'Порядок тот же, что в планировании',
        detail: 'В «Действиях» ходят в том же порядке, что зафиксирован на этот игровой ход.',
      },
      {
        icon: 'build',
        label: 'Постройка по маркеру действия',
        detail: 'Щёлкните маркер, чтобы открыть окно постройки: корабли появляются на клетке маркера, оплата идёт фишками региона.',
      },
      {
        icon: 'marker-action',
        label: 'Фаза идёт, пока на карте есть маркеры',
        detail: 'Фаза не заканчивается, пока на карте есть маркеры действий.',
      },
      {
        icon: 'click',
        label: 'За ход — один маркер или снятие',
        detail:
          'За свой ход можно исполнить один маркер или снять другой без действия — только до исполнения, с подтверждением.',
      },
      {
        icon: 'ship',
        label: 'Щелчок по маркеру: корабли и маршрут',
        detail: 'Щелчок по вашему маркеру открывает выбор кораблей, потом укажите клетки назначения.',
      },
      {
        icon: 'fight',
