...
export const uiStringsRu = {
  lobby: {
    ...
    botLevelHint: {
      easy: 'Играет прямолинейно: занимает ближайшее, первым делом центры власти, и нападает, когда сильнее.',
      medium: 'Развивает экономику и идёт к своим центрам власти по обстановке. Свои центры почти не защищает и чужой победе не мешает.',
      hard: 'Играет на победу: развивает экономику, держит важные центры власти, считает шансы в бою и срывает победу того, кто вот-вот её возьмёт.',
  ...
  cellMarkers: {
    owners: (names: readonly string[]) =>
      names.length > 1
        ? `Маркеры действия: ${names.join(', ')} — у каждой стороны свой`
        : `Маркер действия: ${names[0] ?? ''}`,
  },
  siegeMark: {
