...
describe('beginMatchForParticipants', () => {
  it('strips unused slots and sets planning among joiners', () => {
    ...
    // Бюджет перезарядки выдаётся с первого хода; долг заводится, только если выбирать есть из чего.
    expect(game.rechargePicksRemainingByPlayer).toBeDefined()
  })

  it('does not grant recharge budgets before match start', () => {
    const map = twoStartMap()
    const game = gameSnapshotFromMap(map)
    expect(game.rechargePicksRemainingByPlayer).toBeUndefined()
  })
