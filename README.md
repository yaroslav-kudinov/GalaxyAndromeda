...
- При работе через **tuna/HTTPS-туннель** направляйте туннель на порт **3000** (клиент), не на 3001 напрямую — иначе прокси `/api` не сработает. Запускайте `pnpm dev:tunnel` (или клиент с `NUXT_HMR_TUNNEL=1` / `pnpm --filter @galaxy/client dev:tunnel`), чтобы HMR использовал `wss` на порту `443`.
