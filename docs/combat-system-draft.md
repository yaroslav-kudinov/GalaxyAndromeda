...
|--------|----------|-----|-------|
| **Вход в оспариваемую клетку** | Корабль перемещается на гекс, где уже есть **вражеские корабли** | ✅ | — |
...
**Оспариваемая клетка** для **движения** (`isCombatDestination`):

- на клетке есть корабли другого игрока (любой тип).

Пустая клетка (в т.ч. под чужим контролем) — не бой. Вход на чужой контроль сразу передаёт контроль входящему; нейтраль по-прежнему только объявлением в начале производства (не эсминцем).

**Цель обстрела** (`isBombardmentDestination`): любой вражеский корабль **или** чужой контроль (в т.ч. пустая controlled-клетка).
