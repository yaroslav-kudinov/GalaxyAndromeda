...
**До броска кубов** любой участник боя может объявить skip для **типа корабля** (`ShipType`), а не для отдельного экземпляра. Оба игрока (атакующий и защитник) могут объявить skip независимо, по одному разу на каждый тип, присутствующий на их стороне в бою. Объявление **бесплатное** — без оплаты фишками. Все корабли этого типа на стороне игрока откладываются в **конец своего tier** в `destructionPriority`.
