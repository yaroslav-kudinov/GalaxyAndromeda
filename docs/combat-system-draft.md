...
**До броска кубов** любой участник боя может объявить skip для **типа корабля противника** (`ShipType`), а не для отдельного экземпляра. Оба игрока (атакующий и защитник) объявляют skip независимо, по одному разу на каждый тип, присутствующий у врага. Объявление **бесплатное** — без оплаты фишками. Если объявивший побеждает, все корабли пропущенного типа у проигравшего откладываются в **конец своего tier** и получают +1 к `destroyCost`.
