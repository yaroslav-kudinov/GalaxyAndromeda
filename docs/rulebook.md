...
**Числовые показатели боя:** destroyer имеет `destroyCost` **4**. Корабли в гексе боя бросают свои `combatDice` как d6. Кубики поддержки с дистанции: cruiser — 1d4, battleship — 2d4, Г.О. — 3d4; число `supportDice` и `fireRange` не меняются.

**Движение:** дальность `move` — длина **пути по существующим гексам** карты (BFS по face-adjacent). Через «дыры» (где гекса нет) лететь нельзя. Поддержка / обстрел по `fireRange` пока считаются осевым расстоянием (может уточняться отдельно).
...
**Priority skip:** объявляется до броска, без оплаты фишками, **только на текущий раунд**. Игрок пропускает **типы кораблей противника** строго по `destructionPriority` среди типов, присутствующих в бою (эсминцы → Г.О. → щит → крейсер → линкор → снабжение; отсутствующий тип не учитывается). Нельзя пропустить крейсер, пока не пропущен эсминец (и т.д. по цепочке). Пропуск **последнего** присутствующего типа бессмысленен — после пропуска всех предыдущих уже можно уничтожать любой оставшийся корабль. Если игрок выиграет раунд, каждый пропущенный тип у проигравшего дороже на **+1** к `destroyCost` (линкор 9→10) и его можно обойти при выборе уничтожения (открывается следующий tier).
...
**Кубики поддержки:** cruiser / battleship / Г.О. бросают с дистанции соответственно 1d4 / 2d4 / 3d4. Их `combatDice` на гексе боя остаются d6; поддержка не меняет число кубиков.
...
Колода из 14 карт; порядок вытягивания детерминирован по номеру хода (для тестов и реплеев).
