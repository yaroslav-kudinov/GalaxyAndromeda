...
| `stop-combat` | `{ retreatTo: { q, r } }` | Текущий решающий участник отступает в соседнюю клетку без вражеских кораблей (сначала attacker, затем defender). Защитнику недоступны клетки, куда летят другие корабли того же маркера атакующего (`continuation.movementPlans`). Отложенные перелёты маркера после боя в клетку с чужими кораблями не исполняются — корабль остаётся на месте |
