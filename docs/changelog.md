...
- **Бой — выбор уничтожения:** UI больше не даёт выбрать крейсер/низший тип, пока не выбраны все корабли выше по приоритету (без priority skip); `selectableIds` в rules учитывает приоритет. Карточки выбора с кастомными «галочками», глифом и бюджетом.
