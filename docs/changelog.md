...
- **Карта `maps/duel-2p.json`:** компактная симметричная дуэль на 2 игрока (23 гекса, 3 центра власти, стартовые корабли снабжения и эсминца).
...
- **Захват клетки:** мирный вход на чужой контроль без боевого флота передаёт контроль входящему и снимает чужой маркер производства. То же при победе в бою движением. Отступление контроль назначения не меняет.
...
- **Бой — триггер:** бой только при вражеском корабле не-`supply` (пустая / только снабжение / чужой control без флота — не бой). Обстрел по-прежнему по `isBombardmentDestination` (корабли или чужой control). Отступление не меняет контроль назначения.
